...
$('.shopping-bag > .text').html('<span class="text" style="left:0;top:0">Cart</span><a href="http://sewdemo2.sewingmachinesplus.com/cgi-sewdemo2/sb/order.cgi?storeid=*1a55e44c50ad4a9e18c062cfd94a56&amp;function=show"><img id ="imagcheck" src="http://sewdemo2.sewingmachinesplus.com/AE/images/HCI/checkoutbtn.png"></a>');
...
/*  var un= getCookie('Email');
 alert(un);
$("input[name='email1']").val(un);
var un= getCookie('First');
$("input[name='First']").val(un);
var un= getCookie('Last');
$("input[name='Last']").val(un); */
...
$(window).load(function () {
	...
	$('.shopping-bag > span .text').innerHTML('cart');

//	REMOVE NEXTOPIA UI CSS:
	$('link[href^="//cdn.nextopia.net/"]').remove();
...
//alert (bages1);
  ...
  $('.product-detail-desc .add-buttons').click(function(e) {
	 // e.preventDefault();
    ...
    document.cookie = cname + "=" + cvalue + ";" + expires + ";path=/";
	  ...
	  //alert ('soniya');
$('.badge').html(function(i, val) {
	var count = val*1+1;
	if(isNaN(count)){
		return true;
	}else{
		//alert (count);
		setCookie('cart11',val*1+1,4);
		return  val*1+1;
	}
...
$(document).ready(function () {
  $(function() {
    ...
    start = cookies.indexOf("ss_cart_0000000123="),
    cartvalues = "",
    linecount = 0,
    start1,
    end1,
  ...
  var beged =  $('#header-bag, #header-mobile-bag').find('.badge').text($output);
return;
  });
    });
...
/* $('.product-detail-desc .add-buttons').click(function() {
$('.badge')++;
} */
/* var count = 0;
...
//$('.badge').append('<label>Product111111 Selections</label>')
