...
/*remove 'if' after split test*/
	if(ss_jQuery('.split-test-new-nav:visible').length==0){//if the old menu is being displayed
		ss_jQuery('#expertsewingconsultants').attr('display','block');//show the expert sewing consultant ad
		ss_jQuery('.cmlla').removeClass('new-cmlla');//remove the class that adds a margin on all the content so it's not hidden behind the nav
	}else{//if new menu is displayed
		ss_jQuery('#expertsewingconsultants').css('display','none');//hide the expert sewing consultant ad
		ss_jQuery('.cmlla').addClass('new-cmlla');//add the class that adds a margin on all the content so it's not hidden behind the nav
	}

	if(devBrowser.indexOf('msie')!=-1||devBrowser.indexOf('trident')!=-1){//check if the device is using any IE below Edge
		//ss_jQuery('head').append('<link rel="stylesheet" href="/AE/css/kyle-new-nav-ie.css"/>');//if yes use the stylesheet for IE
		$(".locations-container").addClass( "wtfman" );
	}
...
/*remove 'if' after split test*/
	if(ss_jQuery('.split-test-new-nav:visible').length==0){//if old menu is up
